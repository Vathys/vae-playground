"""
Adapted from https://github.com/IrvingMeng/MagFace by @IrvingMeng
"""
...
def load_features(arch, embedding_size):
    if arch == "iresnet18":
        features = iresnet18(pretrained=True, num_classes=embedding_size)
    elif arch == "iresnet34":
        features = iresnet34(pretrained=True, num_classes=embedding_size)
    elif arch == "iresnet50":
        features = iresnet50(pretrained=True, num_classes=embedding_size)
    elif arch == "iresnet100":
        features = iresnet100(pretrained=True, num_classes=embedding_size)
    else:
        raise ValueError()
    return features
...
def load_magface(path):
    if os.path.isfile(path):
        checkpoint = torch.load(path, map_location=torch.device("cpu"))
    else:
        raise ValueError(f"No checkpoint found at {path}")

    model = MagFace(checkpoint["arch"], 512)

    _state_dict = clean_dict(model, checkpoint["state_dict"])
